...
async def download_file(filename: str, db: db_dependency, current_user: Users = Depends(get_current_user)):
    
    file_record = db.query(FileManage).filter(FileManage.filename == filename, FileManage.user_id == current_user.id).first()
    ...
    return FileResponse(path=file_record.path, filename=file_record.filename, media_type=file_record.file_type)